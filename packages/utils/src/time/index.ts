...
const MILLISECONDS_PER_DAY = 24 * MINUTES_PER_HOUR * SECONDS_PER_MINUTE * MILLISECONDS_PER_SECOND;

export function resolveDayRange(isoDate: string, timeZoneOffsetMinutes: number) {
	const [year, month, day] = isoDate.split("-").map(Number);
	const startUtcMillis = Date.UTC(year ?? 0, (month ?? 1) - 1, day ?? 1);
	const from = new Date(startUtcMillis + timeZoneOffsetMinutes * SECONDS_PER_MINUTE * MILLISECONDS_PER_SECOND);
...
export function secondsBetween(from: Date, to: Date) {
	return Math.max(0, Math.round((to.getTime() - from.getTime()) / MILLISECONDS_PER_SECOND));
}

export function roundSecondsToBillableHours(seconds: number, incrementMinutes: number) {
